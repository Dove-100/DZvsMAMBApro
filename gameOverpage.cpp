 ...
 */
     ...
     */
GameOverScreen::GameOverScreen(const sf::Texture& backgroundTexture, const sf::Texture& playerTexture, const sf::Texture& bossTexture, const sf::Font& font, sf::Vector2u windowSize, int playerhealth)
        ...
        gameOverText(font, "YOU WIN!", 100),// 游戏结束标题
        ...
        credits(font, "Credits: \n.exe team", 20)//制作者名单
{
    ...
    if (playerhealth > 0) {
		gameOverText.setString("YOU WIN!");// 如果玩家生命值大于0，显示胜利
	}
    else {
        gameOverText.setString("YOU LOSE!");// 如果玩家生命值小于等于0，显示失败
    }
}
